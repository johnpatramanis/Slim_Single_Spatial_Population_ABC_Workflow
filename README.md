...
**```Slim_Script.slim```**: This file is equally important to the ```Snakemakefile```. This Slim script is spatial (2D) [non-WF simulation model](https://academic.oup.com/mbe/article/36/3/632/5229931). The template for this simulation was taken from ["Space is place" - Battey et al. 2020](https://academic.oup.com/genetics/article/215/1/193/5930485), but a number of modification have been made. The simulation is controlled by a number of parameters (e.g. simulation duration or size of 2D space). These parameters are preset within the script, but will automatically be substituted by the parameters within ```params.json```.
<br/><br/>
