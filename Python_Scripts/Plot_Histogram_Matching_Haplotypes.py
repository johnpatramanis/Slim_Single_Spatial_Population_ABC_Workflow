...
for ANC in sorted(ANCESTRIES.keys()):
    ...
    for LOC_INFO in Individual_Location:
        ...
        for DATA_HERE in DATA_THIS_ANCESTRY:
            ...
            if ((ID1 == ID_NAME) or (ID2 == ID_NAME)) and (
                DATA_HERE not in SORTED_BY_LOCATION_DATA
            ):
    ...
    ax.set_xlabel(
        'Individual genomes sorted by position on X axis\n(left to right)',
        fontweight='bold',
        fontsize=11,
    )
    ax.set_ylabel(
        'Individual genome sorted by position on X axis\n(top to bottom)',
        fontweight='bold',
        fontsize=11,
    )
    ax.set_title(
        f'Heatmap of similarity of\n Ancestry {ANC} segments',
        fontsize=12,
        pad=14,
        fontweight='bold',
    )
