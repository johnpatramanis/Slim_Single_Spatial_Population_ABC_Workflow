...
Individuals_File = open(
    f"{Folder.replace('Diversity_Metrics', 'Sampled_Individuals.txt')}", 'r'
)
...
max_width = round(
    max(Individual_Location, key=lambda x: x[1])[1]
)  ### maximum X axis position, rounded
max_height = round(
    max(Individual_Location, key=lambda x: x[2])[2]
)  ### maximum Y axis position, rounded
...
print(
    f'Maximum width for this space is: {max_width} and maximum height is:'
    f' {max_height}\n'
)
...
for X_DIM in range(
    Size_of_Box, max_width + Size_of_Box, Size_of_Box
):  ### left to right
  for Y_DIM in range(
      Size_of_Box, max_height + Size_of_Box, Size_of_Box
  ):  ## bottom to up
    ...
    for IND in Individual_Location:
      ...
      if (
          (IND_X >= X_DIM - Size_of_Box)
          and (IND_Y >= Y_DIM - Size_of_Box)
          and (IND_X < X_DIM)
          and (IND_Y < Y_DIM)
      ):  ## individual within boundaries of box
...
for Ancestry in sorted(ANCESTRIES.keys()):
  ...
  for BOX_1 in BOXES:
    for BOX_2 in BOXES:
      ...
      PAIRED_BOXES.append(
          MEAN_SIMILARITY_BOX
      )  ####  X_OF_BOX_1, Y_OF_BOX_1, X_OF_BOX_2, Y_OF_BOX_2
      PAIRED_BOXES_LOCATION.append(
          f'{X_OF_BOX_1}:{Y_OF_BOX_1}\n{X_OF_BOX_2}:{Y_OF_BOX_2}'
      )  ####  X_OF_BOX_1, Y_OF_BOX_1, X_OF_BOX_2, Y_OF_BOX_2
  ...
  ax.set_xticklabels(box_labels, rotation = 90, fontsize = 4)

  # Apply Yaxis ticks and labels
  ax.set_yticks(tick_positions)
  ax.set_yticklabels(box_labels, fontsize = 4)

  ax.set_xlabel('X, Y position of Box', fontweight='bold', fontsize=13)
  ax.set_ylabel('X, Y position of Box', fontweight='bold', fontsize=13)
  ax.set_title(
      f'Heatmap of similarity of\n Ancestry {Ancestry} segments\n grouped in'
      f' Boxes of Size {Size_of_Box}',
      fontsize=12,
      pad=14,
      fontweight='bold',
  )
  ...
  fig.tight_layout()
  plt.savefig(
      f'{Output_Folder}/Ancestry_{Ancestry}_Regional_Similarity_Heatmap_boxsize_{Size_of_Box}.pdf'
  )
  plt.close()